...
> **v0.0.1 - スキャフォールディング。** 以下で説明する実際のCLI/GUIコアは
> 本当に実装され、テストされています。まだ編集したパーツを
> HYDRA-UMC-SERVER 自身の実在する `POST /api/models/submit` カタログへ
> 送り返すことはできません（HYDRA-UMC-EDITOR-URDF が URDF モデルに対して
> 行っているように）し、変換前後のパーツのライブ 3D プレビューもまだ
> 提供していません — どちらも実在する、範囲が明確な今後の作業であり
> （ロードマップ参照）、暗黙のうちに完了したものとして扱ってはいません。

**誠実さの確認 - 今日実際に動くもの：** `model_catalog.py`
（両方のモデルライブラリに対する実在の読み取り専用ディスカバリ）と
`stl_ops.py`（`numpy-stl` による実在の STL 変更 - 変換/置換/削除/追加）
は、実際に生成した実在の STL ファイルに対してテストされており
（`pytest tests/`、21 件のケースが合格）、本エコシステム自身の実在する
...
Qt Quick デスクトップ GUI（`qt_gui.py`、`qml/Main.qml`）は QML エラー
なしでロード・描画されます（`QT_QPA_PLATFORM=offscreen` で検証済み）が、
それ自体の自動テストは持っていません - 実際の Qt イベントループを
駆動することはここでは試みていません。これは HYDRA-UMC-UPDATER 自身の
README がすでに自らの Qt Quick 層について引いているのと同じ誠実さの
境界線です。
...
4つの実在する操作、それぞれディスク上の実際のチェックアウトに対する
...
│   └── main.py               # ディスパッチ：デフォルトは GUI、--cli は categories/models/parts/transform/replace/remove/add 用
...
- 編集/追加したパーツを HYDRA-UMC-SERVER 自身の実在する
  `POST /api/models/submit` カタログエンドポイントへ送り返す機能。
  これは HYDRA-UMC-EDITOR-URDF がすでに URDF モデルに対して使って
  いるのと同じ実在の統合ポイントです。
- 選択したパーツのライブ 3D プレビュー（変換の前後）。第二の独立した
  OpenGL ビューアを作るのではなく、HYDRA-UMC-SUITE 自身の実在する
  `RobotGLRenderer`/メッシュ読み込みコードを再利用します。
...
- **[HYDRA-UMC-SERVER](https://github.com/JuanenRac/HYDRA-UMC-SERVER)** — 本エディタが完成した編集内容を送信する予定の実在する `POST /api/models/submit` エンドポイントを所有しています（ロードマップ参照 - v0.0.1 ではまだ接続されていません）。
